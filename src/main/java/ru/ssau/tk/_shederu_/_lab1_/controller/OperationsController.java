...
public class OperationsController {
    ...
    public ResponseEntity<Map<String, Object>> performOperation(
        ...
        try {
            ...
            if (id1 == null || id1 <= 0 || id2 == null || id2 <= 0) {
                return ResponseEntity.badRequest().body(createErrorResponse("Invalid function IDs"));
            }
            ...
            switch (operation.toLowerCase()) {
                case "add": result = operationService.add(f1, f2); break;
                case "subtract": result = operationService.subtract(f1, f2); break;
                case "multiply": result = operationService.multiply(f1, f2); break;
                case "divide": result = operationService.divide(f1, f2); break;
                default: return ResponseEntity.badRequest().body(createErrorResponse("Unknown operation"));
            }

            Long userId = userRepository.findByLogin(auth.getName()).get().getId();
            TabulatedFunctionDto resultDto = functionService.functionToDto(result, "Result of " + operation, userId);
    ...
    @PreAuthorize("hasAnyRole('CREATOR', 'ADMIN')")
    public ResponseEntity<Map<String, Object>> performOperationAndSave(
            @PathVariable String operation,
            @RequestParam Long id1,
            @RequestParam Long id2,
        ...
        try {
            ...
            Long userId = userRepository.findByLogin(auth.getName()).get().getId();
            TabulatedFunctionEntity savedEntity = functionService.saveFunctionToDb(result, resultName, userId);
    ...
    @PostMapping("/derive")
    @PreAuthorize("hasAnyRole('CREATOR', 'ADMIN')")
    public ResponseEntity<Map<String, Object>> deriveFunction(@RequestParam Long id, Authentication auth) {
        return performDerivation(id, "simple", auth);
    }

    @PostMapping("/derive-average")
    @PreAuthorize("hasAnyRole('CREATOR', 'ADMIN')")
    public ResponseEntity<Map<String, Object>> deriveWithAverage(@RequestParam Long id, Authentication auth) {
        return performDerivation(id, "average", auth);
    }

    @PostMapping("/derive-sync")
    @PreAuthorize("hasAnyRole('CREATOR', 'ADMIN')")
    public ResponseEntity<Map<String, Object>> deriveSynchronously(@RequestParam Long id, Authentication auth) {
        return performDerivation(id, "sync", auth);
    }

    private ResponseEntity<Map<String, Object>> performDerivation(Long id, String method, Authentication auth) {
        try {
            TabulatedFunction f = functionService.loadFunctionFromDb(id);
            TabulatedFunction result;

            switch (method) {
                case "average": result = differentialOperator.deriveWithAverage(f); break;
                case "sync": result = differentialOperator.deriveSynchronously(f); break;
                default: result = differentialOperator.derive(f); break;
            }

            Long userId = userRepository.findByLogin(auth.getName()).get().getId();
            TabulatedFunctionDto resultDto = functionService.functionToDto(result, "Derivative", userId);
    ...
    @PreAuthorize("hasAnyRole('CREATOR', 'ADMIN')")
    public ResponseEntity<Map<String, Object>> deriveAndSave(
